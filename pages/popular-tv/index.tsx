...
export const PopularTV = () => {
  ...
  useEffect(() => {
    const endPoint = `${API_BASE_URL}/tv/popular?api_key=${API_KEY_PUBLIC}&language=${API_DEFAULT_LANGUAGE}&page=${page}`
    fetch(endPoint)
      .then((res) => res.json())
      .then((data) => {
  ...
  return (
    <>
      <Head>
        <title key='title'>Popular Tv Shows - Filmica</title>
        <meta name='description' key='description' content='Popular TV Shows' />
      </Head>
      <section>
